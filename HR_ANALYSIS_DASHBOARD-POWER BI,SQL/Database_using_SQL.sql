...
SELECT employee_id, Age, Department
FROM hr_raw
LIMIT 10;
...
SELECT employee_id, MonthlyIncome, DailyRate, HourlyRate, 
...
SELECT employee_id, PerformanceRating, JobSatisfaction,
...
SELECT employee_id, TotalWorkingYears, YearsAtCompany,
...
SELECT COUNT(*) FROM performance_details;
SELECT COUNT(*) FROM experience_details;
...
SELECT 
    d.department_name,
    COUNT(CASE WHEN a.attrition_status = 'Yes' THEN 1 END) AS attrition_count,
...
SELECT 
    jr.job_role_name,
    AVG(s.monthly_income) AS avg_salary
FROM employees e
JOIN job_details jd ON e.employee_id = jd.employee_id
...
SELECT e.employee_id, s.monthly_income
FROM employees e
JOIN salary_details s ON e.employee_id = s.employee_id
ORDER BY s.monthly_income DESC
LIMIT 5;
